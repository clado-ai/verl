"""rmsnorm_llama@sm90 — chalk autoresearch kernel (one file per layer, per arch).

Cell: rmsnorm_llama@sm90
...
tree is GEMMA-only (every arch/sm90/rmsnorm.py hardcodes ``(1+w)`` and is graded against the
...
``SPEEDUP=<x>`` only after the chalk autoresearch verifier confirms a real win on a real sm90 GPU.
