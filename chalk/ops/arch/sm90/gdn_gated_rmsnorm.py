"""gdn_gated_rmsnorm@sm90 — chalk arch-tuned kernel (one file per layer, per arch).

Cell: gdn_gated_rmsnorm@sm90
...
+ timing + roofline + anti-cheat) on a real sm90 (H100) GPU on 2026-07-04: 1.8026x (fwd+bwd), all
gates green, no cheat flags. Selected by ``chalk.ops.arch.load_entry`` when the running GPU is sm90
...
ANCHOR: 1.8026x divides by a bar that no longer exists, so do not read it as a ratio against the
...
The kernel code below is byte-identical to the code that earned 1.8026x (verified by AST comparison
...
file on sm90, so the anchor becomes the candidate and the cell times the kernel against itself
...
upstream grad. build() returns the entry callable.
"""
...
def _bwd_kernel(X, Z, W, DY, Rstd, DX, DZ, DW, stride, M, N, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
    ...
    for i in range(BLOCK_M):
        ...
        if row < M:
            ...
            c = dy * silu * w
            mean_cxn = tl.sum(c * xn, axis=0) / N
            dx = r * (c - xn * mean_cxn)
            tl.store(DX + off, dx.to(DX.dtype.element_ty), mask=mask)
            acc += dy * silu * xn
    tl.atomic_add(DW + cols, acc, mask=mask)
