"""rmsnorm_llama@sm86 — chalk autoresearch kernel (one file per layer, per arch).

Cell: rmsnorm_llama@sm86
...
tree is GEMMA-only (every arch/sm86/rmsnorm.py hardcodes ``(1+w)`` and is graded against the
...
``SPEEDUP=<x>`` only after the chalk autoresearch verifier confirms a real win on a real sm86 GPU.
